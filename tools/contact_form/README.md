...
- **リポジトリは public なので Issue も公開される**。Issue に入れるのは種別・対象 URL・AI の要約と
  判定理由・受付日時だけで、氏名・メールアドレス・問い合わせの原文は入れない。要約と題名にも
  個人情報を書かせない（`classifyTool_` の説明文とシステムプロンプトで指示）。
  原文と連絡先は運営者への通知メールと回答スプレッドシートにだけ残る
