...
13. **`CLOUDFLARE_API_TOKEN` に「Account Analytics: Read」を足す**（2026-09-23 起票）。
    週次の「広告のクリック（飛び先ごと）」がこれで出る。いまは配置用の権限だけなので 403 になり、
    週次には「足すと出る」という案内が出る。当面は Cloudflare の Web Analytics の画面で
    `/go/` のページ別表示数を見る。
