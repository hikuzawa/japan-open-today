...
## レビュー待ち（AI が判定できなかったもの。S4 時点で 5 件）
`data/review/kagawa.yaml` に一覧がある。いずれも「運営主体を示す記述を機械的に取得できなかった」もので、
**承認すれば巡回対象になる**。試した情報源は同ファイルの `reason` に書いてある。

| 対象 | 判断してほしいこと |
|---|---|
| 地中美術館 / ベネッセハウス ミュージアム / 豊島美術館 | `benesse-artsite.jp` がこれらの施設の公式サイトであることの承認（運営: 公益財団法人福武財団） |
| 四国汽船 | `shikokukisen.com` が四国汽船の公式サイトであることの承認（直島・豊島航路の中心） |
| 高松城跡 玉藻公園 | `takamatsujyo.com` は robots.txt を取得できない。高松市の公式ページに情報源を移すか、このまま対象外にするか |

承認は 1 件 1 分程度で、`data/review/kagawa.yaml` の `decision` に「承認」と書くだけ（S6 で取り込みを自動化する）。
