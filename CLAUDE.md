...
- `.github/workflows/` 日次パイプライン（pipeline.yml, 05:00 JST）、週次まとめ（weekly.yml）、push ごとの検査（checks.yml）。
  sitemill はタグ固定（現在 `v0.7.4`）。上げるときは 3 本の `ref:` を揃える
...
- **日付から決まる表示には `data-sitemill-volatile` を付ける**（今日の判定・週の帯・今日開いている一覧・件数・
  「○月○日の状況」）。サイトマップの lastmod が「中身が変わった日」になるための印で、付け忘れると
  全ページが毎日「今日変わった」になる（sitemill ADR 0025）。新しい表示を足したら、ビルドの実行レポートの
  `lastmod_changed` がほぼ全ページになっていないかを見る
